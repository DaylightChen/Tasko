...
> A personal task, todo, and planning tracker inspired by TickTick — capture, organize, and stay on top of what matters.
...
| `plan` | Break work into tasks, define execution order. Coverage extends to UX deliverables when present. | `docs/plan/implementation-plan.md` (project) or `docs/features/<name>/plan.md` (feature) |
| `implement` | Execute tasks, write tests, verify, commit | `docs/tasks/` (project) or `docs/features/<name>/tasks/` (feature) |
...
`docs/STATUS.md` is a **human-readable mirror** of the project scope's state, regenerated by commands. Agents read the JSON, not the markdown.
...
5. Record important decisions in the scope's decisions log (project: `docs/decisions.md`; feature: `docs/features/<name>/decisions.md`) with rationale.
6. Log known issues in `docs/known-issues.md` when deferring bugs or workarounds.
...
- One doc per task in `docs/tasks/` (project) or `docs/features/<name>/tasks/` (feature), including execution logs and completion markers.
- Each task writes a `completion.md` after its commit lands — required for the implement phase to be considered complete.
- Each feature scope has its own `docs/features/<name>/STATUS.md` mirror.
